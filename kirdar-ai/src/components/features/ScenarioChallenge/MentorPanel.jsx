...
Objectives: ${scenarioData.objectives?.join(', ')}`;
      ...
      const messages = [
        {
          role: 'system',
          content: `You are an expert ${scenarioData.domain || 'financial'} mentor reviewing a professional-client interaction. Analyze the recent exchanges and provide constructive feedback.

Context:
${contextString}

Focus on:
1. Professional standards and best practices in ${scenarioData.domain || 'financial'} care
2. Communication effectiveness and empathy
3. Addressing client needs and concerns appropriately

Provide:
1. Any compliance/regulatory warnings specific to ${scenarioData.domain || 'financial'} practice
...
${relevantMessages.map(msg => 
  `${msg.role === 'user' ? 'Professional' : 'Client'}: ${msg.content}`
).join('\n\n')}`
        }
      ];
